    ...
    <div className="space-y-6 text-slate-900 font-sans selection:bg-amber-400 selection:text-slate-950 pb-12">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 text-slate-900 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="font-serif text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <form onSubmit={handlePostQuestion} className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-3">
            ...
            />
            <button type="submit" className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-md transition">
          ...
          <div className="space-y-4">
            {questions.map(q => (
              <div key={q.id} className="bg-white rounded-3xl p-6 border border-slate-200 shadow-xs space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    <h4 className="font-serif font-black text-slate-900 text-base hover:text-amber-600 cursor-pointer">{q.title}</h4>
                    <p className="text-xs text-slate-500 mt-0.5 font-medium">{q.author} • {q.batch}</p>
                  </div>
                  {q.verified && (
                    <span className="px-3 py-1 bg-amber-50 text-amber-900 border border-amber-200 text-[10px] font-extrabold rounded-full flex items-center gap-1">
                      <ShieldCheck className="w-3.5 h-3.5 text-amber-600" /> Trainer Verified Answer
                    </span>
                  )}
                </div>
